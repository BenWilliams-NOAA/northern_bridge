...
post <- extract_samples(mcmc2, as.list=TRUE)
...
proj_bio(report3) # lower values than orig
proj_bio(report2) # lower values than orig
proj_bio(report)
report3$M # M down a bit
report$M
report3$q # q up a bit
report$q
...
# Cole recommends 5 chains, 1000 iters, 250 warmup
mcmc <- sample_sparse_tmb(obj3, iter=1000, warmup=250,
                          init='random', chains=5,
...
vrs = seq(log(0.1), log(2.5), length.out = 150)
lq = seq(log(0.7), log(1.2), length.out = 50)
nll = single_like(par_name='log_q', par_values=vrs, obj=obj3, fit=fit3)
plot(vrs, nll, type='l')
lm = seq(log(0.01), log(0.2), length.out = 50)
nll = single_like(par_name='log_M', par_values=lm, obj=obj3, fit=fit3)
plot(lm, nll, type = 'l')
...
par_ranges <- list(
  log_a50C = seq(0, 3.1, length.out=50),  # Replace with appropriate range
  log_a50S = seq(0, 3.1, length.out=50)   # Replace with appropriate range
)
str(obj3)
str(fit3)
fit3$evaluations
# selectivity intercepts
# Profile the likelihood
...
par_ranges <- list(
  deltaC = seq(1, 7, length.out=50),  # Replace with appropriate range
  deltaS = seq(1, 7, length.out=50)   # Replace with appropriate range
)
...
report1$M
report1$q
vals = seq(log(0.111), log(2), length.out = 50)
...
chains <- 5
# Cole recommends 5 chains, 1000 iters, 250 warmup
mcmc <- sample_sparse_tmb(obj1, iter=1000, warmup=250,
